...
output_dir = os.path.join('results', 'lstm_tuned')
...
def plot_model_metrics(accuracy, precision, recall, f1, output_path):
    ...
    plt.title('LSTM Model Performance Metrics with Tuned Hyperparameters', fontsize=16)
...
if __name__ == "__main__":
    ...
    print(f"Tuned LSTM Model - Loss: {tuned_loss}, Accuracy: {tuned_accuracy}")
    
    print("\n--- Detailed Performance Metrics for Tuned Model---")
    ...
    # Calculate and display tuned model metrics
    ...
    print(f"\nActual epochs trained - Tuned LSTM: {actual_epochs}")
    ...
    # Create and save the performance metrics bar chart
    plot_model_metrics(
        tuned_accuracy, 
        tuned_precision, 
        tuned_recall, 
    ...
    print(f"\nTuned LSTM Analysis completed at: {end_time}")
